...
#include <Private/Library/PeiPchIshLib.h>
  ...
  )
{
  ...
  if (PchIsPcieImrEnabled ()) {
    BiosImrDisableMask0 &= (UINT32)~(BIT7);  // PCH PCIe
    PcieImrSize = PchGetPcieImrSize ();
  }
#ifndef CPU_CFL
  if (SaIsPcieImrEnabled ()) {
    BiosImrDisableMask0 &= (UINT32)~(BIT7);  // CPU PCIe
    PcieImrSize = SaGetPcieImrSize ();
  }
#endif
  if (IsHdAudioEnabled ()) {
  ...
  if (IsIpuEnabled ()) {
    ...
    if (IpuConfiguration == SA_IMR_IPU_CAMERA) {
...
#ifdef UP_SERVER_FLAG
  if (EFI_ERROR(Status)) {
    // Request default value in case HECI communication problem
    DEBUG ((DEBUG_ERROR, "UMA size is set to default, Status = %r\n", Status));
    *ImrsSize  = 0x1000000;
    *Alignment = 0x1000000;
    DEBUG ((DEBUG_ERROR, "  IMR Size = 0x%08x\n",          *ImrsSize));
    DEBUG ((DEBUG_ERROR, "  Largest IMR Size = 0x%08x\n",  *Alignment));
  }
#endif
