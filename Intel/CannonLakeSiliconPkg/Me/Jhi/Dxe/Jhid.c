  ...
  )
{
  ...
  if ((mAppHandle->VersionInfo.FwMajor >= 7 && mAppHandle->VersionInfo.FwMajor <= 10) ||
    mAppHandle->VersionInfo.FwMajor == 1 || mAppHandle->VersionInfo.FwMajor == 2) {
    MaxAppletCount = MAX_APPLETS_BH1;
  } else if (mAppHandle->VersionInfo.FwMajor == 11 || mAppHandle->VersionInfo.FwMajor == 12) {
    MaxAppletCount = MAX_APPLETS_BH2;
  } else {
    MaxAppletCount = MAX_APPLETS_BH2;
  }
  ...
  )
{
  ...
  if (rc != JHI_SUCCESS) {
    ...
    if (pRxBuf->buffer != NULL) {
      ...
      AsciiStrToUnicodeStrS ((CHAR8 *) pRxBuf->buffer, (CHAR16 *) pCommBuffer->RxBuf->buffer, pCommBuffer->RxBuf->length);
  ...
  )
{
  ...
  if ((mAppHandle->VersionInfo.FwMajor >= 7 && mAppHandle->VersionInfo.FwMajor <= 10) ||
    mAppHandle->VersionInfo.FwMajor == 1 || mAppHandle->VersionInfo.FwMajor == 2) {
    MaxSessionCount = MAX_SESSIONS_BH1;
  } else if (mAppHandle->VersionInfo.FwMajor == 11 || mAppHandle->VersionInfo.FwMajor == 12) {
    MaxSessionCount = MAX_SESSIONS_BH2_GEN1;
  } else {
    MaxSessionCount = MAX_SESSIONS_BH2_GEN2;
  }
