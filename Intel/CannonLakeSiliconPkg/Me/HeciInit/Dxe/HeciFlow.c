...
HECI_FLOW_CONTEXT gFlowContext;

HECI_FLOW_PROTOCOL gHeciFlowProtocol;
...
/**
  ...
  @retval       EFI_SUCCESS if connection was created, error otherwise
**/
EFI_STATUS
CreateConnection (
...
/**
  ...
  @retval       EFI_SUCCESS if address pair was found, error otherwise
**/
EFI_STATUS
GuidToAddressPair (
...
/**
  ...
  @param[in]  Length           length of message in buffer
  @param[in]  Guid             GUID of the client inside CSME
  @retval     EFI_SUCCESS if message could be sent, error otherwise
**/
EFI_STATUS
EFIAPI
Send (
...
/**
  Function to register gBS->ExitBootServices() and LegacyBoot callbacks for disconnecting all HECI Flow connections
**/
VOID
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "InitializeHeciFlow\n"));
  gHeciFlowProtocol.SendAndReceive = SendAndReceive;
  gHeciFlowProtocol.Send           = Send;
