...
Field(RPCF,DWordAcc,NoLock,Preserve)
{
    ...
    Offset(0xE2),
    ,       2,
    L23E,   1,                      // L23_Rdy Entry Request (L23ER)
    L23R,   1,                      // L23_Rdy to Detect Transition (L23R2DT)
}
...
Method(HGON,0,Serialized)
{
    ...
    SGPO(HRE0, HRG0, HRA0, 0)                  //Deassert dGPU_HOLD_RST# {Release the dGPU Reset} as per the PCIe spec
    ...
    Store(0, Local0)
    // Once in Detect, wait up to 124 ms for Link Active
    // 24ms in Detect, 72ms in Polling, 28ms in Config
    While(LEqual(LASX,0)) {
...
Method(HGOF,0,Serialized)
{
    ...
    // Save the Link Control register, DGPU SSID and put the link into L2/L3
    Store(LCTR,ELCT)
    Store(SVID,HVID)
    Store(SDID,HDID)
