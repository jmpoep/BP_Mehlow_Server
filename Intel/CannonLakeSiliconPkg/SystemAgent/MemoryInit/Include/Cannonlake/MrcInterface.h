...
#define MAX_RANK_IN_LPDRAM                  (4)       ///< The maximum number of ranks per LPDDR DRAM
#define MAX_RANK_IN_CHANNEL                 (MAX_DIMMS_IN_CHANNEL * MAX_RANK_IN_DIMM) ///< The maximum number of ranks per channel.
#define MAX_BANKS                           (16)      ///< Max Number of Banks supported.  This is a combination of BankGroup and Bank.
...
#define SUPPORT_LPDDR                       (SUPPORT_LPDDR4)
...
// SSKPD_PCU_CNL_SKPD BIT MASK
#define SSKPD_PCU_CNL_SKPD_TRAIN_DONE       0x1
#define SSKPD_PCU_CNL_SKPD_MEM_TEST_DONE    0x2
...
typedef enum {
  ...
  OemDdr4TxVrefRange,       ///<  before Ddr4 TxVref Range adjustment.
...
typedef struct {
  UINT32 TeRsv0   : 1;   ///< BIT0  - CFL Rsv
  UINT32 TeRsv1   : 1;   ///< BIT1  - CFL Rsv
  UINT32 TeRsv2   : 1;   ///< BIT2  - CFL Rsv
  UINT32 TeRsv3   : 1;   ///< BIT3  - CFL Rsv
  ...
  UINT32 TXDQVREF : 1;   ///< BIT7 - TxDqVref Range Training
  UINT32 RsvdBits :24;
} TrainingStepsEn2;
...
typedef enum {
  cmCNL_ULX_ULT   = CPUID_FULL_FAMILY_MODEL_CANNONLAKE_ULT_ULX, ///< Cannonlake ULT/ULX
  cmCNL_DT_HALO   = CPUID_FULL_FAMILY_MODEL_CANNONLAKE_DT_HALO, ///< Cannonlake DT/HALO
} MrcCpuModel;
...
typedef enum {
  ...
  csCnlC0         = EnumCnlC0,
  csCnlD0         = EnumCnlD0,
  csCnlUlxUltLast = csCnlD0,

  /// Cannonlake Dt/Halo
  csCnlP0         = EnumCnlP0,
  csCnlG0         = EnumCnlG0,
  csCnlDtHaloLast = csCnlG0
} MrcCpuStepping;
...
// CNL Full bios doesn't use the MrcCalltable provided Io read and write functions. CNL Mini bios uses them.
...
//MrcSdramOut Sdram[MAX_SDRAM_IN_DIMM];       ///< The following are SDRAM level definitions.
  UINT16      MR[MAX_SUB_CHANNEL][MAX_MR_IN_DIMM];  ///< DRAM mode register values, indexed by MrcModeRegisterIndex enum.
...
typedef struct {
  ...
  MrcRankOut     Rank[MAX_RANK_IN_LPDRAM];  ///< The following are rank level definitions.
} MrcDimmOut;
...
typedef struct {
  ...
  MrcTurnaroundTimes  tWr2Wr;                                                       ///< The system's minimal delay timings for Read  command followed by Read  command.
  MrcTurnaroundTimes  tRd2Rd;                                                       ///< The system's minimal delay timings for Read  command followed by Write command.
  MrcTurnaroundTimes  tRd2Wr;                                                       ///< The system's minimal delay timings for Write command followed by Read  command.
  MrcTurnaroundTimes  tWr2Rd;                                                       ///< The system's minimal delay timings for Write command followed by Write command.
} MrcChannelOut;
...
#define MRC_REGISTER_COUNT_COMMON (1396 / sizeof (UINT32)) ///< The number of MC registers that need to be saved (common)
#define MRC_REGISTER_COUNT_SAGV   (2148 / sizeof (UINT32)) ///< The number of MC registers that need to be saved (per SA GV point)
typedef struct {
  ...
  BOOLEAN         Any2Dpc;                                  ///< Indicates that the configuration was 2DPC during training boot.
  BOOLEAN         DramDqOdtEn;                              ///< Specifies if the DRAM DQ ODT is enabled.
  BOOLEAN         ODTCaCsCkSupport;                         ///< Indicate if need to set reserved bit for ODT CA/CS/CK support
...
typedef struct {
  ...
  UINT8               BurstLength;                 ///< Burst length in DCLKs
  BOOLEAN             EnhancedChannelMode;         ///< Indicates if 64-bit channel is subdivided into two 32-bit channels.
  BOOLEAN             EnhancedRankMode;            ///< Indicates if Enhanced Rank Mode is enabled.
...
typedef struct {
  ...
  UINT32  Off64Bits6to8Rsvd:3;            ///<  - Bits 6-8 Reserved.
  ...
  UINT32  SharedZqPin:1;                  ///<  - Must be set if the ZQ resistor is shared between Ranks in DRAM Packages <b>0=Not Shared</b>, 1=Shared
  UINT32  Off64Bits12to16Rsvd:5;          ///<  - 5 Bits reserved [Bits 12-16] for Thermal Management - CFL
  UINT32  Lp4DqsOscEn:1;                  ///<  - <CNL> LpDdrDqDqsReTraining support: 0=Disable, <b>1=Enable</b>
  UINT32  DualDimmPerChannelBoardType:1;  ///<  - Bit 18 reserved for CFL
  UINT32  ReservedBits1:13;               ///<  - Bits 19:31 reserved
  /**
  ...
  **/
  ...
  UINT8   Reserved1;              ///< Offset 79 Reserved for CFL
  // Training Algorithms
  ...
  UINT32  DqPinsInterleaved:1;        ///<  - Interleaving mode of DQ/DQS pins which depends on board routing: <b>0=Disable</b>, 1=Enable
  ...
  UINT32  Reserved5:1;                ///<  - Reserved for CFL
  ...
  UINT8   Reserved2[6];           ///< Offset 102 - 107 Reserved for CFL
  ...
  UINT8   Reserved3[3];           ///< Offset 226-228: ConfigBlock size must be a multiple of DWORDs (Offset 226-228: Reserved for CFL)
  ...
  UINT32  Offset232RsvBit3to7:5;  ///< Offset 232 Bit 3-7: Reserved for CFL
  ...
  UINT8  Off252Rsvd[8];           ///< Offset 252 - 268 Reserved for CFL
