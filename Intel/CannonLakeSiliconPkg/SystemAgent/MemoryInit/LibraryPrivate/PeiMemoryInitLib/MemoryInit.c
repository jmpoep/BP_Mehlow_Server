...
//
#define RES_MEM_LIMIT_1 ((UINTN) PcdGet64 (PcdPciExpressBaseAddress) - 1)
#endif // FSP_FLAG
...
DEBUG_CODE_END();
    ...
    case bmS3:
      if (SaveDataValid == FALSE) {
        ...
        if ((ColdBootRequired (MrcData, MemConfig, MrcBootMode) == TRUE) ||
            (((MrcWmRegGet(MrcData) & SSKPD_PCU_CNL_SKPD_MEM_TEST_DONE) == 0) && (MrcBootMode == bmWarm)))  {
          DEBUG ((
            DEBUG_WARN,
  ...
  //
  // If Bclk reset occured during S3, we need to issue IMR message for
  // proper ME functionality.
  //
  MeImrRequiredOnS3 = FALSE;
  if(PmcIsBclkS3Boot ()) {
    DEBUG ((DEBUG_INFO, "IMR Configuration required on S3 for OC flow.\n"));
    MeImrRequiredOnS3 = TRUE;
    PmcClearBclkS3 ();
  }

  if ((MrcBootMode != bmS3) || (MeImrRequiredOnS3 == TRUE)) {
    ImrsSize       = 0;
    Alignment      = 0;
  ...
  if ((MrcBootMode == bmCold) || (MrcBootMode == bmFast)) {
    ...
    if (mrcFail == BasicMemoryTest (MrcData)) {
      if (MrcBootMode == bmFast) {
        if (Inputs->RetrainOnFastFail) {
          ...
          MrcWmRegClrBits(MrcData, SSKPD_PCU_CNL_SKPD_MEM_TEST_DONE);
    ...
    MrcWmRegSetBits(MrcData, SSKPD_PCU_CNL_SKPD_MEM_TEST_DONE);
  } else {
    //
...
//@todo - Move EDKII non-FSP capsule outside of FSP code.
  ...
  if (TopUseableMemSize.Data > MEM_EQU_4GB) {
    ...
    //
    while ((TopUseableMemSize.Data - MEM_EQU_4GB) > MEM_EQU_4GB) {
      DEBUG ((DEBUG_INFO, "Found 0x100000000 bytes at 0x%016lX\n", TopUseableMemAddr));

      if (Inputs->MemoryTrace && (TopUseableMemAddr + MEM_EQU_4GB > RShiftU64 (Tom.Data, 1))) {
        //
      ...
      TopUseableMemSize.Data = TopUseableMemSize.Data - MEM_EQU_4GB;
  ...
  )
{
  ...
  *NumRanges  = 0;
  ...
  if (SiPreMemPolicyPpi != NULL) {
    MemConfig = NULL;
    GetConfigBlock ((VOID *) SiPreMemPolicyPpi, &gMemoryConfigGuid, (VOID *) &MemConfig);
    if (MemConfig != NULL) {
      //
  ...
  )
{
  ...
  /**
    ...
    @note Since IED memory is part of TSEG, re-calculate TSEG size with IED size.
    Also TSEG size must be power of 2, ex., 4MB, 8MB, 16M, 32MB.
     When re-calculating TSEG less or equal than 8MB and IED required,
     then the TSEG is increased to the next power of 2 to provide enough SMM space.
     Otherwise, IED is shared with SMM drivers so TSEG isn't too large.
  **/

  Inputs->TsegSize = GetPowerOfTwo32 (MiscPeiPreMemConfig->TsegSize + MiscPeiPreMemConfig->IedSize) >> 20; // This sets lower 2^n.
  if (MiscPeiPreMemConfig->TsegSize <= 0x800000 && MiscPeiPreMemConfig->IedSize > 0) {
    Inputs->TsegSize <<= 1; // According to policy, set to higher 2^n.
  }
  ...
  Inputs->VccIomV = 950;
  if (UlxUlt) {
    DmiBar = (MmioRead32 (Inputs->PciEBaseAddress + DMIBAR_0_0_0_PCI_CNL_REG)) & ~BIT0;
...
DEBUG_CODE_END();
  ...
  //
  // Enable P-Path from PCH B-step onwards
  //
  if ((IsPchLp () && (Inputs->PchStepping >= PCH_B0)) || (Inputs->CpuModel == cmCNL_DT_HALO)) {
    Inputs->Lp4Npath  = FALSE;
  } else {
    Inputs->Lp4Npath  = TRUE;
  }
  ...
  Inputs->TrainingEnables2.TXDQVREF = 1;
        ...
        if ((DIMM_ENABLED == DimmIn->Status) || (DIMM_DISABLED == DimmIn->Status)) {
          if (DimmIn->SpdAddress > 0) {
            if ((bmCold == BootMode) || (bmFast == BootMode)) {
              MrcCall->MrcGetSpdData (BootMode, DimmIn->SpdAddress, (UINT8 *) &DimmIn->Spd.Data, (UINT8 *) &SpdDdr3Table, sizeof (SpdDdr3Table), (UINT8 *) &SpdDdr4Table, sizeof (SpdDdr4Table), (UINT8 *) &SpdLpddrTable, sizeof (SpdLpddrTable));
            }
          } else {
  ...
  )
{
  ...
  switch (CpuModel) {
    case cmCNL_ULX_ULT:
      Inputs->CpuModel   = cmCNL_ULX_ULT;
      Inputs->CpuFamily  = cfCnl;
      Inputs->TestEngine = MrcTeCpgc15;
      DEBUG ((DEBUG_INFO, "Cannonlake ULT/ULX:"));

      switch (CpuStepping) {
        case csCnlB0:
          Inputs->CpuStepping = csCnlB0;
          Step = "B0";
          break;

        case csCnlC0:
          Inputs->CpuStepping = csCnlC0;
          Step = "C0";
          break;

        case csCnlD0:
          Inputs->CpuStepping = csCnlD0;
          Step = "D0";
          break;

        default:
          Inputs->CpuStepping = csCnlUlxUltLast;
          break;
      }
      Status = mrcSuccess;
      break;
    ...
    case cmCNL_DT_HALO:
      Inputs->CpuModel   = cmCNL_DT_HALO;
      Inputs->CpuFamily  = cfCnl;
      Inputs->TestEngine = MrcTeCpgc15;
      DEBUG ((DEBUG_INFO, "Cannonlake DT/HALO:"));

      switch (CpuStepping) {
        case csCnlP0:
          Inputs->CpuStepping = csCnlP0;
          Step = "P0";
          break;

        case csCnlG0:
          Inputs->CpuStepping = csCnlG0;
          Step = "G0";
          break;

        default:
          Inputs->CpuStepping = csCnlDtHaloLast;
          break;
      }
      Status = mrcSuccess;
      break;
  ...
  )
{
  ...
  switch (Outputs->DdrType) {
    case MRC_DDR_TYPE_DDR4:
      MemoryInfo->MemoryType = MemoryTypeDdr4;
      break;

    case MRC_DDR_TYPE_LPDDR4:
        MemoryInfo->MemoryType = MemoryTypeLpddr4;
        break;
