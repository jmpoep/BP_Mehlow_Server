...
typedef struct {
  ...
  UINT32 Stop[MrcReutFieldMax];    ///< (4, uint16)    // Rank, Bank, Row, Col
...
typedef struct {
  MRC_PAT_SRC_TYPE PatSource;       ///< Defines the source of patterns: Static = WDB, Dynamic = PG
  UINT16 IncRate;                   ///< How quickly the pattern changes: WDB walks through cachelines, or PG rotates.  This is a linear count.
  UINT8  Start;                     ///< Starting pointer in WDB
  UINT8  Stop;                      ///< Stopping pointer in WDB
...
extern MRC_BG_BANK_PAIR Ddr4x8BankMapDg[MAX_BANKS];
extern MRC_BG_BANK_PAIR Ddr4x16BankMapDg[MAX_BANKS];
extern MRC_BG_BANK_PAIR Ddr4BankMapSg[MAX_BANKS];
///
...
/**
  ...
  @retval MrcStatus - mrcSuccess if we enter Self-Refresh, else mrcFail.
**/
extern
MrcStatus
EnterSR (
...
/**
  ...
  @param[in] Chunks   - Length of the Pattern array in chunks (Pattern array first dimension)
  @param[in] Start    - Starting cacheline in the WDB to write to.
  @param[in] NumCL    - Total number of cachelines to write. If Pattern is shorter than this, it will cycle from the start (cyclic).
...
WriteWDBFixedPattern (
  IN MrcParameters *const MrcData,
  IN const UINT32         Pattern[][2],
  IN const UINT8          Chunks,
  IN const UINT8          Start,
  IN const UINT8          NumCL
  );
...
/**
  ...
  Note: This function requires that MrcSelectPatternGen() has been called.
...
/**
  ...
  Note2: This function requires that MrcSelectPatternGen() has been called.
...
MrcProgramSequenceAddress (
  ...
  IN const UINT32         StopAddr[MrcReutFieldMax],
...
SetupIOTest (
  ...
  IN MRC_PATTERN_CTL *const       WDBPattern,
...
/**
  ...
  @param[in,out] MrcData       - Include all MRC global data.
  @param[in]     ChannelMask   - Channel mask to update Vref for.
  ...
  @param[in]     IsCacheOffset - Determines if the parameter is an offset (relative to cache) or absolute value. Relevant only for internval Vref.
...
/**
  ...
  @param[in]     GroupMask  - which LPDDR groups to work on for CMD/CLK/CKE;
