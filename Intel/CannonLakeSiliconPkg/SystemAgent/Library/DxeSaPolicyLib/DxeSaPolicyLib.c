...
/**
  RmrrCsmeBaseAddress buffer allocation for WiAMT DMA

  @param[in] MiscDxeConfig      Points to MiscDxeConfig
**/
STATIC
VOID
UpdateRmrrCsmeBaseAddress (
  IN    MISC_DXE_CONFIG   *MiscDxeConfig
  )
{
  EFI_STATUS            Status;
  EFI_PHYSICAL_ADDRESS  CsmeMemAddress;
  UINTN                 CsmeMemSize;
  ...
  //
  // Initalize Csme Base Address
  //
  MiscDxeConfig->RmrrCsmeBaseAddress = AllocateZeroPool (sizeof (EFI_PHYSICAL_ADDRESS) * 2);

  if (MiscDxeConfig->RmrrCsmeBaseAddress != NULL) {
    ...
    if (MbpHob != NULL) {
      if (MbpHob->MeBiosPayload.FwPlatType.RuleData.Fields.IntelMeFwImageType == IntelMeCorporateFw) {
        CsmeMemSize = EFI_SIZE_TO_PAGES(SA_VTD_RMRR_CSME_LENGTH);
        CsmeMemAddress = MAX_UINT32;
        ...
        if (EFI_ERROR (Status)) {
          DEBUG ((DEBUG_ERROR, "CsmeMemAddress allocate pool failed\n"));
          MiscDxeConfig->RmrrCsmeBaseAddress[0] = 0;
          MiscDxeConfig->RmrrCsmeBaseAddress[1] = 0;
        } else {
          MiscDxeConfig->RmrrCsmeBaseAddress[0] = CsmeMemAddress;
          MiscDxeConfig->RmrrCsmeBaseAddress[1] = CsmeMemAddress + SA_VTD_RMRR_CSME_LENGTH - 1;
        }
      }
    }
  } else {
    DEBUG ((DEBUG_ERROR, "RmrrCsmeBaseAddress allocate pool failed\n"));
  }
}

/**
  Returns the RmrrCsmeBaseAddress for WiAMT DMA

  @retval UINT32        The base address of RMRR that allocate for WiAMT DMA
**/
UINT32
GetRmrrCsmeBaseAddress (
  VOID
  )
{
  EFI_STATUS            Status;
  ...
  return (UINT32)MiscDxeConfig->RmrrCsmeBaseAddress[0];
}
  ...
  )
{
  ...
  ///
  /// RMRR Base and Limit Address for WiAMT DMA
  ///
  UpdateRmrrCsmeBaseAddress (MiscDxeConfig);
