  ...
  )
{
  ...
  if ((((MmioRead16 ((UINTN) (DmiBar + R_SA_DMIBAR_LSTS_OFFSET))) & 0x0F) < MaxDmiSpeed) ||
	...
	/// PCH LCTL2.TLS written need to go through DMI link and it is much slower compared to 
	/// writing CPU LCTL.RL bit and need to be reflected in PCH LCTL2.TLS Before CPU LCTL.RL
	/// (Link Training is set) bit is set. The below 'for' loop iteration is done to ensure
	/// that PCH LCTL2.TLS is set as desired before link training is initiated.
	for (LoopCount = 0; LoopCount < DMI_LINK_RETRIES; LoopCount++) {
	  ValueRead = MmioRead16(PCH_PCR_ADDRESS (PID_DMI, R_PCH_DMI15_PCR_LCTL2)); 
      if (((ValueRead & B_PCH_DMI15_PCR_LCTL2_TLS) == DMI_GEN3) || (LoopCount == DMI_LINK_RETRIES - 1)) {
        DmiLinkTrain (DmiBar);
	    break;
      } 
    } 
  ...
  )
{
  ...
  SklPegDmiRecipe (
    DmiBar.Data,
    MchBar.Data,
    &(PciePeiPreMemConfig->DmiGen3RxCtlePeaking[0]),
    SA_DMI_MAX_BUNDLE,
    0,
    TRUE
    );
  ...
  )
{
  ...
  if (((CpuSku  == EnumCpuHalo) || (CpuSku  == EnumCpuTrad)) && (CpuSteppingId == EnumCnlP0)) {
      StateTracker = 0;
      for (LoopCount = 0; LoopCount < 1000; LoopCount++) {
        StateTracker = (MmioRead32 ((UINTN)(DmiBar + R_SA_PEG_REUT_PH1_PIS_OFFSET))) & 0x3F;
        if (StateTracker == 0x19 || StateTracker == 0x1A || StateTracker == 0x1B || StateTracker == 0x1C || StateTracker == 0x1D) { // These values indicate that the
          break;
        }
      ///
        /// Retrain link
