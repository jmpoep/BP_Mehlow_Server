  ...
  **/
  ...
  for (Dimm = 0; Dimm < SA_MC_MAX_SOCKETS; Dimm++) {
    ...
    if ((mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].Status == DIMM_PRESENT) && (mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].DimmCapacity > 0)) {
      DramDeviceType = mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].SpdDramDeviceType;
  ...
  **/
  for (Dimm = 0; Dimm < SA_MC_MAX_SOCKETS; Dimm++) {
    ...
    if ((mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].Status == DIMM_PRESENT) && (mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].DimmCapacity > 0)) {
      ...
      ///
      DramDeviceType = mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].SpdDramDeviceType;
      ModuleType = mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].SpdModuleType;
      MemoryBusWidthByte = mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].SpdModuleMemoryBusWidth;

      ///
      /// MRC save/restore hob only provides these SPD bytes: Non-JEDEC LPDDR3/DDR3 - 117 ~ 145; JEDEC LPDDR3/DDR4 - 320 ~ 348
      ///
      SmbusBuffer = (UINT8 *) &(mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].SpdSave);
...
#else
      /// In SPD spec, Bits [4:3] Bus width extension, in bits:
      ///                    00 = 0 bits (no extension)
      ///                    01 = 8 bits
      /// So the TotalWidth should be "DataWidth + 8" if the extension bit present.
      MemoryTotalWidth = MemoryDataWidth + 8 * (((MemoryBusWidthByte & 0x18) == 0x08)? 1 : 0);
#endif
      SmbiosTableType17Data.TotalWidth = MemoryTotalWidth;
      SmbiosTableType17Data.DataWidth = MemoryDataWidth;

      DimmMemorySizeInMB = mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].DimmCapacity;
      ///
      ...
      SmbiosTableType17Data.MemoryType = mMemInfo->MemoryType;
      ...
      SmbiosTableType17Data.Speed                      = mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].Speed;
      SmbiosTableType17Data.Attributes                 = mMemInfo->Controller[0].ChannelInfo[ChannelIndex].DimmInfo[DimmIndex].RankInDimm & 0x0F;
      SmbiosTableType17Data.ConfiguredMemoryClockSpeed = mMemInfo->ConfiguredMemoryClockSpeed;
