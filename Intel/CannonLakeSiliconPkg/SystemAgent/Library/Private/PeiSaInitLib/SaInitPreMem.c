...
/**
  ...
  @param[in]  MchBarAddress      The MCHBAR address.
  @param[in]  RegBarAddress      The REGBAR address.

**/
VOID
PcieImrWa (
  IN UINT32 MchBarAddress,
  IN UINT32 RegBarAddress
  )
{
  //
  // W/A for letting PCIe can access PCIe IMR
  //
  if (GetCpuFamily() == EnumCpuCnlUltUlx) {
    ASSERT ((MmioRead32 (MchBarAddress + R_SA_MCHBAR_REGBAR_OFFSET) & BIT0) == BIT0);
    MmioOr32 (RegBarAddress + 0x4058, BIT24);
  }
}
#endif
  ...
  )
{
  ...
  if (PchIsPcieImrEnabled ()) {
    ///
    /// Perform PCIe IMR W/A
    ///
    PcieImrWa (MiscPeiPreMemConfig->MchBar, MiscPeiPreMemConfig->RegBar);
  }
#endif
  ...
  ///
  MmioOr32 ((UINTN) MchBar.Data + 0x701C, BIT22);
#endif
  ...
  )
{
  ...
  /// Program SA GdxcBar
  ///
  MmioWrite64 ((UINTN) (MiscPeiPreMemConfig->MchBar) + R_SA_MCHBAR_GDXCBAR_OFFSET,
    (UINT64) (MiscPeiPreMemConfig->GdxcBar | BIT0));
#else
  ///
  /// Program SA RegBar
  ///
  MmioWrite64 ((UINTN) (MiscPeiPreMemConfig->MchBar) + R_SA_MCHBAR_REGBAR_OFFSET,
    (UINT64) (MiscPeiPreMemConfig->RegBar | BIT0));
#endif
  ...
  if (AsmReadMsr64 (MSR_PLATFORM_INFO) & B_PLATFORM_INFO_EDRAM_EN) {
    ...
    /// Program SA EdramBar
    ///
    MmioWrite64 ((UINTN) (MiscPeiPreMemConfig->MchBar) + R_SA_MCHBAR_EDRAMBAR_OFFSET,
      (UINT64) (MiscPeiPreMemConfig->EdramBar | BIT0));
  }
}
