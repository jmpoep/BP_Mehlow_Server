  ...
  )
{
  ...
  if (PcdGetBool (PcdIpuEnable)) {
    IpuPreMemPolicy->SaIpuEnable = TRUE;
  }
#endif
}
...
#ifndef CPU_CFL
  PciePeiPreMemConfig->PegImrEnable                   = FALSE;
  PciePeiPreMemConfig->PegImrSize                     = 0;
  PciePeiPreMemConfig->ImrRpSelection                 = 1;
#endif
...
#else
  ///
  /// Initialize the CdClock to 528 Mhz
  ///
  GtConfig->CdClock             = 2;
#endif
}
...
#ifndef CPU_CFL
VOID
LoadCpuTraceHubDefault (
  IN VOID    *ConfigBlockPointer
  )
{
  CPU_TRACE_HUB_CONFIG   *CpuTraceHubConfig;
  CpuTraceHubConfig = ConfigBlockPointer;
  DEBUG ((DEBUG_INFO, "CpuTraceHubConfig->Header.GuidHob.Name = %g\n", &CpuTraceHubConfig->Header.GuidHob.Name));
  DEBUG ((DEBUG_INFO, "CpuTraceHubConfig->Header.GuidHob.Header.HobLength = 0x%x\n", CpuTraceHubConfig->Header.GuidHob.Header.HobLength));
}
#endif
  ...
  )
{
  ...
  if (PcdGetBool (PcdSimicsEnable)) {
    ...
    // CNL SA GV: 0 - Disabled, 1 - FixedLow, 2 - FixedMid,  3 - FixedHigh, 4 - Enabled
#ifdef CPU_CFL
    MemConfig->SaGv             = 3;
#else
    MemConfig->SaGv             = 4;
...
#ifndef CPU_CFL
  ,{&gCpuTraceHubConfigGuid,          sizeof (CPU_TRACE_HUB_CONFIG),       CPU_TRACEHUB_CONFIG_REVISION,        LoadCpuTraceHubDefault}
#endif
};
