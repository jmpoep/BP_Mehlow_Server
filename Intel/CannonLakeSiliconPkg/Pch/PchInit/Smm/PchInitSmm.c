  ...
  )
{
  ...
  if(((Pm1Cnt & B_ACPI_IO_PM1_CNT_SLP_TYP) == V_ACPI_IO_PM1_CNT_S5 ||
    ...
    XhciBar0Low = PciSegmentRead32 (XhciPciBase + R_XHCI_CFG_BAR0);
    XhciBar0High = PciSegmentRead32 (XhciPciBase + R_XHCI_CFG_BAR0 + 4);
    ...
    //Take XHCI to D0
    PciSegmentAnd8 (XhciPciBase + R_XHCI_CFG_PWR_CNTL_STS, (UINT8)~B_XHCI_CFG_PWR_CNTL_STS_PWR_STS);
    //Enable MSE
    PciSegmentOr16 (XhciPciBase + PCI_COMMAND_OFFSET, EFI_PCI_COMMAND_MEMORY_SPACE);
    //Set 0x80E0[25] = 1 to force the cmd_ssv flop to 1 internal to xHCI.
    MmioOr32 (XhciMmioBase + R_XHCI_MEM_AUX_CTRL_REG1, BIT25);
    //Set 0x80E0[25] = 0 to remove the force.  But cmd_ssv flop is already set to 1 in previous step.
    MmioAnd32 (XhciMmioBase + R_XHCI_MEM_AUX_CTRL_REG1, (UINT32)~BIT25);
    //Restore BAR and MSE
    PciSegmentAnd16 (XhciPciBase + PCI_COMMAND_OFFSET, (UINT16) ~EFI_PCI_COMMAND_MEMORY_SPACE);
    PciSegmentWrite32 (XhciPciBase + R_XHCI_CFG_BAR0, XhciBar0Low);
    PciSegmentWrite32 (XhciPciBase + R_XHCI_CFG_BAR0 + 4, XhciBar0High);
