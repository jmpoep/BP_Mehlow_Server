...
/**
  Error handling for the case SMBus devices hold SMBDAT line low

  @param [in]    SmbusRegBase    Smbus pci base address
**/
STATIC
VOID
SmbusDataLowErrorHandler (
  UINT64                      SmbusRegBase
  )
{
  //
  // Based on SMBus spec 4.2.5 SMBDAT low timeout:
  // It's possible that a malfunctioning device holds the SMBDAT line low indefinitely.
  // This would prevent the master from issuing a STOP condition and ending a transaction.
  // At this time there is no specification on the maximum time that a device can hold
  // SMBDAT low after the master raises SMBCLK after the last bit of a transaction.
  // Such a specification is under consideration for future revisions of the SMBus specification.
  // In the meantime, the recommendation is that if SMBDAT is still low t(TIMEOUT.MAX)
  // after SMBCLK has gone high at the end of a transaction the master should hold SMBCLK
  // low for at lesat t(TIMEOUT.MAX) in an attempt to reset the SMBus interface of all of
  ...
  //
  // Check the error condition by:
  // Normal case:
  //   1. SMBC.SMBDATA_CUR_STS (bit1) = 1
  //   2. SMBC.SMBCLK_CUR_STS (bit0) = 1
  //   3. HSTS.HBUSY (bit0) = 0
  // Error case:
  //   1. SMBC.SMBDATA_CUR_STS (bit1) = 0
  //   2. SMBC.SMBCLK_CUR_STS (bit0) = 1
  //   3. HSTS.HBUSY (bit0) = 0
  //
  Hsts = SmbusIoRead (R_SMBUS_IO_HSTS);
  Smbc = SmbusIoRead (R_SMBUS_IO_SMBC);

  //
  // Clear Status Register before anyone uses the interfaces
  //
  if (((Smbc & B_SMBUS_IO_SMBDATA_CUR_STS) == 0) &&
      ((Smbc & B_SMBUS_IO_SMBCLK_CUR_STS) != 0) &&
      ((Hsts & B_SMBUS_IO_HBSY) == 0))
  {
    //
    // Force reset SMBus:
    // 1. SMBC.SMBCLK_CTL (bit2) = 0
    // 2. HCFG.SSRESET (bit3) = 1
    // 3. Wait for 35ms
    // 4. SMBC.SMBCLK_CTL (bit2) = 1
    //
    SmbusIoWrite (R_SMBUS_IO_SMBC, 0);
    PciSegmentOr8 (SmbusRegBase + R_SMBUS_CFG_HOSTC, B_SMBUS_CFG_HOSTC_SSRESET);
    MicroSecondDelay (35000);
    SmbusIoWrite (R_SMBUS_IO_SMBC, B_SMBUS_IO_SMBCLK_CTL);
  }
  ...
  )
{
  ...
  // Error handling for the case SMBus devices hold SMBDAT line low
  //
  SmbusDataLowErrorHandler (SmbusRegBase);
