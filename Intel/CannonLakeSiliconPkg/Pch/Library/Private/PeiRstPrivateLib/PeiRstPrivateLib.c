...
GLOBAL_REMOVE_IF_UNREFERENCED PCH_SATA_CONFIG                *mSataConfig;
  ...
  )
{
  ...
  return (IsHybridStorageDevice (DeviceVendorId, DeviceDeviceId));
}
  ...
  )
{
  ...
  SataRegBase = GetSataRegBase (SATA_1_CONTROLLER_INDEX);
  ...
  for (CrNumber = 0; CrNumber < PCH_MAX_RST_PCIE_STORAGE_CR; CrNumber++) {
    if (RstSetCycleRouter (SataRegBase, CrNumber)) {
      ...
      //
      PciSegmentOr32 (SataRegBase + R_SATA_CFG_RST_PCIE_STORAGE_EGCR,
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "RstConfigurePcieStorageRemapping: RstConfigurePcieStorageDeviceAspm() Started\n"));
  ...
  RootportDownstreamPmConfiguration (
    ...
    TempPciBusMin,
    TempPciBusMax,
    &mPcieRpConfig->RootPort[RpNumber],
    NumOfDevAspmOverride,
    DevAspmOverride
    );

  DEBUG ((DEBUG_INFO, "RstConfigurePcieStorageRemapping: RstConfigurePcieStorageDeviceAspm() Ended\n"));
}
  ...
  )
{
  ...
  for (RpNumber = 0; RpNumber < GetPchMaxPciePortNum (); RpNumber++) {
    ...
    if (mSataConfig->RstPcieStorageRemap[CrNumber].Enable == 0) {
      DEBUG ((DEBUG_INFO, "RstMarkCycleRoutersForRemapping: Cycle router %d is disabled by policy\n", CrNumber));
      RpNumber += 3 - (RpNumber % 4);
      continue;
    }

    if ((mSataConfig->RstPcieStorageRemap[CrNumber].RstPcieStoragePort != 0) &&
        (mSataConfig->RstPcieStorageRemap[CrNumber].RstPcieStoragePort != RpNumber + 1)) {
  ...
  )
{
  ...
  DEBUG_CODE_BEGIN ();
  DEBUG ((DEBUG_INFO, "=====================================================\n"));
  DEBUG ((DEBUG_INFO, "RstConfigurePcieStorageRemapping: RstPcieStorageRemappingLateConfig() Dump\n"));
  ...
  ///
  PciSegmentAndThenOr32 (SataRegBase + R_PCH_PCIE_CFG_DCTL2 + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET,
  ...
  if ((mSavedConfigSpace[CrNumber].EndpointLctlData16 & B_PCIE_LCTL_CCC) == B_PCIE_LCTL_CCC) {
    ...
    PciSegmentOr16 (SataRegBase + R_PCH_PCIE_CFG_LCTL + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET, (UINT16) (B_PCIE_LCTL_RL));

    RstPollPcieLinkStateUntil (TRUE, 30000, SataRegBase + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET + R_PCH_PCIE_CFG_LSTS);
  }
  ...
  if (mRstCrConfig[CrNumber].MsixConfig.MsixSupported) {
    Data32 = (mRstCrConfig[CrNumber].MsixConfig.EndingVector << N_SATA_CFG_RST_PCIE_STORAGE_CR1_DMXC_MXEV) | mRstCrConfig[CrNumber].MsixConfig.StartingVector;
    MmioWrite32 (AhciBar + R_SATA_CFG_RST_PCIE_STORAGE_CR1_DMXC + (CrNumber * V_SATA_CFG_RST_PCIE_STORAGE_REMAP_CONFIG_CR), Data32);
    MmioOr32 (AhciBar + R_SATA_CFG_RST_PCIE_STORAGE_CR1_DMXC + (CrNumber * V_SATA_CFG_RST_PCIE_STORAGE_REMAP_CONFIG_CR) + 0x04, BIT31);
  } else {
    ///
    ...
    ///
    MmioWrite32 (AhciBar + R_SATA_CFG_RST_PCIE_STORAGE_CR1_DMXC + (CrNumber * V_SATA_CFG_RST_PCIE_STORAGE_REMAP_CONFIG_CR), 0);
    MmioWrite32 (AhciBar + R_SATA_CFG_RST_PCIE_STORAGE_CR1_DMXC + (CrNumber * V_SATA_CFG_RST_PCIE_STORAGE_REMAP_CONFIG_CR) + 0x04, 0);
  }
  ...
  ///
  Data32 = (mRstCrConfig[CrNumber].ClassCode.BaseClassCode << N_SATA_CFG_RST_PCIE_STORAGE_CR1_DCC_BCC) | (mRstCrConfig[CrNumber].ClassCode.SubClassCode << N_SATA_CFG_RST_PCIE_STORAGE_CR1_DCC_SCC) | (mRstCrConfig[CrNumber].ClassCode.ProgrammingInterface);
  if (mRstCrConfig[CrNumber].ClassCode.ProgrammingInterface == RST_PCIE_STORAGE_INTERFACE_AHCI) {
    Data32 |= B_SATA_CFG_RST_PCIE_STORAGE_CR1_DCC_DT;
  }

  ///
  /// Program the endpoint "Device Class Code", AHCIBar + offset 800h + n*80h with the 32-bit data
  ///
  MmioWrite32 (AhciBar + R_SATA_CFG_RST_PCIE_STORAGE_CR1_DCC + (CrNumber * V_SATA_CFG_RST_PCIE_STORAGE_REMAP_CONFIG_CR), Data32);
  ...
  ///
  Data16 = PciSegmentRead16 (SataRegBase + R_PCH_PCIE_CFG_LSTS + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET);
  ...
  ///
  MmioWrite32 (AhciBar + R_SATA_CFG_RST_PCIE_STORAGE_CR1_DMBL + (CrNumber * V_SATA_CFG_RST_PCIE_STORAGE_REMAP_CONFIG_CR), mRstCrConfig[CrNumber].EndPointBarSize);
  ...
  DEBUG ((DEBUG_INFO, "RstConfigurePcieStorageRemapping: RstPcieStorageRemappingLateConfig() Ended\n"));
}
  ...
  )
{
  ...
  DEBUG_CODE_BEGIN ();
  DEBUG ((DEBUG_INFO, "=====================================================\n"));
  DEBUG ((DEBUG_INFO, "RstConfigurePcieStorageRemapping: RstPcieStorageRemappingEarlyConfig() Dump\n"));
  RstPcieStorageRemappingDump (CrNumber);
  DEBUG ((DEBUG_INFO, "=====================================================\n"));
  DEBUG_CODE_END ();
  ...
  DeviceBarSize = RstProgramRemapBarAndGetItsSize (EndPointPciBase, SataRegBase, CrNumber);

  if (!RstCheckDeviceBarSize (DeviceBarSize, CrNumber, EndPointPciBase)) {
  ...
  ///
  if (!(*RemapBarEnabled)) {
    Data32 = (V_SATA_CFG_VS_CAP_NRMO << N_SATA_CFG_VS_CAP_NRMO) | (B_SATA_CFG_VS_CAP_NRMBE) | (V_SATA_CFG_VS_CAP_MSL << N_SATA_CFG_VS_CAP_MSL);
    MmioWrite32 (AhciBar + R_SATA_CFG_VS_CAP, Data32);
  ...
  ///
  PciSegmentOr32 (SataRegBase + PCI_COMMAND_OFFSET + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET, EFI_PCI_COMMAND_BUS_MASTER);

  ///
  /// Set all "Memory Base" bits in MBL, PBML and PMBU32 to 1
  ///
  PciSegmentOr32 (SataRegBase + R_PCI_BRIDGE_MBL + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET, B_PCI_BRIDGE_MBL_MB);
  PciSegmentOr32 (SataRegBase + R_PCI_BRIDGE_PMBL + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET, B_PCI_BRIDGE_PMBL_PMB);
  PciSegmentOr32 (SataRegBase + R_PCI_BRIDGE_PMBU32 + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET, B_PCI_BRIDGE_PMBU32);

  ///
  /// Re-enable PCIe link
  ///
  PciSegmentAnd32 (SataRegBase + R_PCH_PCIE_CFG_LCTL + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET, (UINT32) ~(B_PCIE_LCTL_LD));
  DEBUG ((DEBUG_INFO, "RstConfigurePcieStorageRemapping: RstPcieStorageRemappingEarlyConfig() Ended\n"));
}
  ...
  )
{
  ...
  ///
  if ((mSataConfig->Enable == FALSE) &&
      (RemapEnabled == TRUE)) {
    DEBUG ((DEBUG_ERROR, "RstConfigurePcieStorageRemapping: Can't perform RST PCIe Storage Remapping when Sata Controller is Disabled\n"));
    ASSERT (FALSE);
    return FALSE;
  }
  ...
  )
{
  ...
  for (CrNumber = 0; CrNumber < PCH_MAX_RST_PCIE_STORAGE_CR; CrNumber++) {
    ...
    //
    if (mSataConfig->RstPcieStorageRemap[CrNumber].DeviceResetDelay > MaxDeviceResetDelay) {
      MaxDeviceResetDelay = mSataConfig->RstPcieStorageRemap[CrNumber].DeviceResetDelay;
    }

    if (RstSetCycleRouter (SataRegBase, CrNumber)) {
      RstPollPcieLinkStateUntil (TRUE, 1000 * 1000, SataRegBase + V_SATA_CFG_RST_PCIE_STORAGE_REMAP_RP_OFFSET + R_PCH_PCIE_CFG_LSTS);
    }
  }
  ...
  )
{
  ...
  MmioWrite32 (AhciBar + R_SATA_CFG_VS_CAP, (0x00F << N_SATA_CFG_VS_CAP_NRMO));
  ...
  )
{
  ...
  return RstIsPcieStorageRemapEnabledByConfig (SataConfig->RstPcieStorageRemap);
}
  ...
  )
{
  ...
  mSataConfig                 = GetPchSataConfig (SiPolicy, SATA_1_CONTROLLER_INDEX);
  RemapCapReg                 = 0;
  ...
  RstCreateCycleRouterMap (SataRegBase, mCycleRouterMap);
  RstDetectPcieStorageDevices (SataRegBase, TempPciBusMin);
  RstInitPcieStorageInfoHob ();

  RstConfigurePmForRemapping ();

  if (!RstIsRemappingPossible (RstIsPcieStorageRemapEnabled (SiPolicy, SATA_1_CONTROLLER_INDEX), SataRegBase)) {
    return;
  }
  ...
  PciSegmentWrite32 (SataRegBase + R_SATA_CFG_AHCI_BAR, AhciBar);
  ...
  if (!RstIsRemappingRequired ()) {
    ...
    ///
    for (Index = 0; Index < PCH_MAX_RST_PCIE_STORAGE_CR; Index++) {
      RstDisablePcieStorageRemapping (SataRegBase, Index);
    }
    goto Exit;
  }
  ...
  for (Index = 0; Index < PCH_MAX_RST_PCIE_STORAGE_CR; Index++) {
    ...
    /// Configure the ASPM for Root Port and PCIe storage device before enabling RST PCIe Storage Remapping
    ///
    RstConfigurePcieStorageDeviceAspm (Index, TempPciBusMin, PcdGet8 (PcdSiliconInitTempPciBusMax));
    ...
    ///
    RstPcieStorageRemappingEarlyConfig (SataRegBase, AhciBar, Index, SataPortsEnabled, &RemapBarEnabled, TempPciBusMin);
  }

  RstReenablePcieLinkForRemapedDevices (SataRegBase);
  ...
  for (Index = 0; Index < PCH_MAX_RST_PCIE_STORAGE_CR; Index++) {
    ...
    ///
    RemapCapReg |= B_SATA_CFG_RST_PCIE_STORAGE_RCR_NRS_CR1 << Index;
  }

  ///
  ///  Program "Remap Capability Register", AHCIBar offset 800h [2:0] based on enabled CR
  ///
  MmioWrite8 (AhciBar + R_SATA_CFG_RST_PCIE_STORAGE_RCR, RemapCapReg);
  ...
  ///
  MmioWrite16 (AhciBar + R_SATA_CFG_RST_PCIE_STORAGE_AMXC, RemapDeviceLinkInfo);

  RstEnableLegacyInterruptServicing (SataRegBase);

  ///
  /// Perform lockdown programming for unused Cycle Router
  ///
  RstLockdownUnusedPcieStorageCycleRouter (SataRegBase);
