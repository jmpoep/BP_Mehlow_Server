  ...
  )
{
  ...
  //
  PciSegmentOr32 (GbePciBase + R_LAN_CFG_LANDISCTRL, B_LAN_CFG_LANDISCTRL_DISABLE);
  ASSERT (PciSegmentRead32 (GbePciBase) != 0xFFFFFFFF);

  //
  // Set LOCKLANDIS, GbE PCI config + 0xA4 bit[0] to 1
  //
  PciSegmentOr32 (GbePciBase + R_LAN_CFG_LOCKLANDIS, B_LAN_CFG_LOCKLANDIS_LOCK);
  ...
  )
{
  ...
  if (LanConfig->LtrEnable == TRUE) {
    PciSegmentWrite32 (GbePciBase + R_LAN_CFG_LTR_CAP, 0x10031003);
  }

  ///
  /// Configure GbE interrupt
  ///
  InterruptPin = ItssGetDevIntPin (SiPolicy, PCI_DEVICE_NUMBER_PCH_LAN, PCI_FUNCTION_NUMBER_PCH_LAN);
  ...
  PciSegmentWrite32 (GbePciBase + R_LAN_CFG_MBARA, GbeMemBar);
  ...
  if (PmConfig->WakeConfig.WolEnableOverride) {
    MmioOr32 (GbeMemBar + R_LAN_MEM_CSR_FEXT, B_LAN_MEM_CSR_FEXT_WOL | B_LAN_MEM_CSR_FEXT_WOL_VALID);
  } else {
    MmioAndThenOr32 (GbeMemBar + R_LAN_MEM_CSR_FEXT, (UINT32) ~(B_LAN_MEM_CSR_FEXT_WOL), B_LAN_MEM_CSR_FEXT_WOL_VALID);
  }

  if (IsClkReqAssigned (PchClockUsageLan)) {
    MmioOr32 (GbeMemBar + R_LAN_MEM_CSR_FEXTNVM6, B_LAN_MEM_CSR_FEXTNVM6_K1OFF_EN);
  ...
  //
  MmioOr32 (GbeMemBar + R_LAN_MEM_CSR_PBECCSTS, B_LAN_MEM_CSR_PBECCSTS_ECC_EN);
  MmioOr32 (GbeMemBar + R_LAN_MEM_CSR_CTRL, B_LAN_MEM_CSR_CTRL_MEHE);

  //
  // Disable memory space decoding
  //
  PciSegmentAnd16 (GbePciBase + PCI_COMMAND_OFFSET, (UINT16) ~EFI_PCI_COMMAND_MEMORY_SPACE);
  PciSegmentWrite32 (GbePciBase + R_LAN_CFG_MBARA, 0);
}
