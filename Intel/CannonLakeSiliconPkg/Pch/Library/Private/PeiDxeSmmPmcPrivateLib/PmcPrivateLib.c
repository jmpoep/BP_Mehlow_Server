...
{
  return MmioRead8 (PmcGetPwrmBase () + R_PMC_PWRM_BIOS_SCRATCHPAD);
}
  ...
  )
{
  ...
  if (!PmcIsIpcCmdA9Supported ()) {
    DEBUG ((DEBUG_INFO, "This PMC version doesn't support CMD A9\n"));
    return;
  }
  ...
  if (DisableMe) {
    Status = PmcSendCommand (V_PMC_PWRM_IPC_ME_DISABLE, 1, 0, &Wbuf, NULL);
  } else {
    Status = PmcSendCommand (V_PMC_PWRM_IPC_ME_DISABLE, 0, 0, &Wbuf, NULL);
  }
  ASSERT_EFI_ERROR (Status);
}
/**
  ...
  )
{
  ...
  ZeroMem (&Rbuf, sizeof (PMC_IPC_COMMAND_BUFFER));
  //
  // WBUF0 = 1 for 8254CGE bit
  //
  Wbuf.Buf0 = V_PMC_PWRM_IPC_CMD_WBUF0_PROXY_8254CGE;
  Status = PmcSendCommand (
             V_PMC_PWRM_IPC_CMD_COMMAND_PROXY,
             V_PMC_PWRM_IPC_CMD_CMD_ID_PROXY_READ,
             4,
  ...
  ASSERT_EFI_ERROR (Status);
  *ClockGeteState = (Rbuf.Buf0 & B_ITSS_PCR_ITSSPRC_8254CGE) != 0;
  return Status;
}
