  ...
  )
{
  ...
  MmioAnd32 (Context->ConfigAddress + R_SERIAL_IO_CFG_PME_CTRL_STS, (UINT32) (~B_SERIAL_IO_CFG_PME_CTRL_STS_PWR_ST));
  MmioOr32 (Context->MmioAddress + R_SERIAL_IO_MEM_PPR_RESETS,
  ...
  //
  if (MmioRead32 (Context->MmioAddress + R_IC_GENERAL) & (B_IC_GENERAL_SDA_LINE_STATE | B_IC_GENERAL_SCL_LINE_STATE)) {
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  if ( *BusClockHertz < 100*1000) {
    ...
    *BusClockHertz = 100000;
    ...
    MmioWrite32 ( MmioAddress + R_IC_SS_SCL_HCNT, 1010);
    MmioWrite32 ( MmioAddress + R_IC_SS_SCL_LCNT, 1085);
  } else if ( *BusClockHertz < 1000*1000) {
    //for any request in range [400kHz, 1MHz), set 400kHz
    *BusClockHertz = 400000;
    Speed = V_IC_SPEED_FAST;
    MmioWrite32 ( MmioAddress + R_IC_SDA_HOLD, 0x001E001E);
    MmioWrite32 ( MmioAddress + R_IC_FS_SCL_HCNT, 193);
    MmioWrite32 ( MmioAddress + R_IC_FS_SCL_LCNT, 313);
  } else if ( *BusClockHertz < 3400*1000) {
    //for any request in range [1MHz, 3.4MHz), set 1MHz
    *BusClockHertz = 1000000;
    Speed = V_IC_SPEED_FAST;
    MmioWrite32 ( MmioAddress + R_IC_SDA_HOLD, 0x001E001E);
    MmioWrite32 ( MmioAddress + R_IC_FS_SCL_HCNT, 61);
    MmioWrite32 ( MmioAddress + R_IC_FS_SCL_LCNT, 132);
  } else {
    //for any request in range [3.4MHz, +inf), set 3.4MHz
    *BusClockHertz = 3400000;
    Speed = V_IC_SPEED_HIGH;
    MmioWrite32 ( MmioAddress + R_IC_SDA_HOLD, 0x001E001E); //todo
    MmioWrite32 ( MmioAddress + R_IC_FS_SCL_HCNT, 19);
    MmioWrite32 ( MmioAddress + R_IC_FS_SCL_LCNT, 11);
  }
