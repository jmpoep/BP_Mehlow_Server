  ...
  )
{
  switch (Vc) {
    case DmiVcTypeVc0:
      *DmiVcCtlAddress = R_PCH_DMI14_PCR_V0CTL;
      *DmiVcStsAddress = R_PCH_DMI14_PCR_V0STS;
      break;

    case DmiVcTypeVc1:
      *DmiVcCtlAddress = R_PCH_DMI14_PCR_V1CTL;
      *DmiVcStsAddress = R_PCH_DMI14_PCR_V1STS;
      break;

    case DmiVcTypeVcm:
      *DmiVcCtlAddress = R_PCH_DMI14_PCR_VMCTL;
      *DmiVcStsAddress = R_PCH_DMI14_PCR_VMSTS;
      break;
  ...
  )
{
  ...
  /// Step 7.1 - 7.4
  ///
  PchPcrAndThenOr32 (PID_OPIPHY, R_PCH_OPIPHY_PCR_B104, ~(UINT32) 0x0, BIT3);

  PchPcrAndThenOr32 (PID_OPIPHY, R_PCH_OPIPHY_PCR_B10C, ~(UINT32) 0x0, BIT31);

  if ((PchPcrRead32 (PID_OPIPHY, R_PCH_OPIPHY_PCR_0110) & (BIT21 | BIT20)) == BIT20) {
    PchPcrAndThenOr32 (PID_OPIPHY, R_PCH_OPIPHY_PCR_B10C, ~(UINT32) 0x0, (BIT6 | BIT5));
  }

  PchPcrAndThenOr32 (PID_OPIPHY, R_PCH_OPIPHY_PCR_0118, ~(UINT32) 0x0, (BIT13 | BIT12 | BIT8));

  PchPcrAndThenOr32 (PID_OPIPHY, R_PCH_OPIPHY_PCR_011C, ~(UINT32) 0x0, BIT14);
  return;
}
  ...
  )
{
  ...
  if (IsPchWithPdmi ()) {
    ///
    /// Step 8.2
    /// Set PCR[DMI] + Offset 232Ch[20] = 1b
    ///
    Data32And = 0xFFFFFFFF;
    Data32Or = BIT20;
    PchPcrAndThenOr32 (
  ...
  if (!IsPchWithPdmi () && (DmiConfig->DmiAspmCtrl != PchPcieAspmDisabled)) {
    ///
    /// Step 8.15
  ...
  ///
  /// Set PCR[DMI] + 2400h[15:8] = 0x60 for PCH-LP < B0
  ///
  ...
  )
{
  ...
  if (PchGeneralConfig->LegacyIoLowLatency) {
    ...
    DmiAspmCtrl = DmiConfig->DmiAspmCtrl;
  }
