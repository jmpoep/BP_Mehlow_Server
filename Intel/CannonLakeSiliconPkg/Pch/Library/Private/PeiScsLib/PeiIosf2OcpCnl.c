...
{
  //
  // On CNL SCS has 3 bridges. One with SdCard controller,
  // one with eMMC and one with UFS.
  //
  Iosf2OcpConfigurePowerAndClockGating (PID_SDX, 0x81DE17);
  Iosf2OcpEnableSnoopedTransactions (PID_SDX);
  if (IsPchLp ()) {
    Iosf2OcpConfigurePowerAndClockGating (PID_EMMC, 0x1D847);
    Iosf2OcpEnableSnoopedTransactions (PID_EMMC);
    Iosf2OcpConfigurePowerAndClockGating (PID_UFSX2, 0x1DFF80F);
