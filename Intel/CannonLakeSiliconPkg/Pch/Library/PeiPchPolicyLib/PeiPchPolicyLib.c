  ...
  )
{
  ...
  ********************************/
  ...
  for (Index = 0; Index < GetPchMaxPcieClockNum (); Index++) {
    PcieRpConfig->PcieClock[Index].Usage = (UINT8)PchClockUsageNotUsed;
    PcieRpConfig->PcieClock[Index].ClkReq = GetClkReqForClkSrc ((UINT8)Index);
  }
  ...
  )
{
  ...
  /********************************
    Lan configuration
  ********************************/
  if (IsCnlPch () && IsPchH () && (PchStepping () == PCH_A1) && ((LpcDid == V_CNL_PCH_H_LPC_CFG_DEVICE_ID_A312_SKU) || (LpcDid == V_CNL_PCH_H_LPC_CFG_DEVICE_ID_A313_SKU))) {
    LanConfig->Enable             = FALSE;
  } else {
  ...
  )
{
  ...
  ********************************/
  ...
  ThermalConfig->PchHotLevel                 = 0x154;  //UPServer
...
**/
VOID
LoadTetonGlacierConfigDefault (
  IN VOID          *ConfigBlockPointer
  )
{
  TETON_GLACIER_CONFIG  *TetonGlacierConfig;
  TetonGlacierConfig = ConfigBlockPointer;

  DEBUG ((DEBUG_INFO, "TetonGlacierConfig->Header.GuidHob.Name = %g\n", &TetonGlacierConfig->Header.GuidHob.Name));
  DEBUG ((DEBUG_INFO, "TetonGlacierConfig->Header.GuidHob.Header.HobLength = 0x%x\n", TetonGlacierConfig->Header.GuidHob.Header.HobLength));

  /********************************
    Teton Glacier configuration.
  ********************************/
  if (IsPchH ()) {
    TetonGlacierConfig->TetonGlacierCR = 0; // Setting the default cycle router to cycle router 0 for CNP-H system
  } else {
    TetonGlacierConfig->TetonGlacierCR = 1; // Setting the default cycle router to cycle router 1 for CNP-LP system
  }
...
GLOBAL_REMOVE_IF_UNREFERENCED COMPONENT_BLOCK_ENTRY  mPchIpBlocks [] = {
  ...
  {&gTetonGlacierConfigGuid,     sizeof (TETON_GLACIER_CONFIG),         TETON_GLACIER_CONFIG_REVISION,     LoadTetonGlacierConfigDefault}
};
