...
/**
  Print TETON_GLACIER_CONFIG.

  @param[in] TetonGlacierConfig         Pointer to a TETON_GLACIER_CONFIG that provides the TG setting

**/
VOID
PchPrintTetonGlacierConfig (
  IN CONST TETON_GLACIER_CONFIG   *TetonGlacierConfig
  )
{
  DEBUG ((DEBUG_INFO, "------------------ TETON GLACIER Config ------------------\n"));
  DEBUG ((DEBUG_INFO, " Teton Glacier Mode                        = %x\n", TetonGlacierConfig->TetonGlacierMode));
  DEBUG ((DEBUG_INFO, " Teton Glacier connected to Cycle Router   = %x\n", TetonGlacierConfig->TetonGlacierCR));
}
...
DEBUG_CODE_BEGIN();
  ...
  PchPrintTetonGlacierConfig (TetonGlacierConfig);
