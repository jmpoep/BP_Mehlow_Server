...
//
// Note: This code is a derivative of Ia32CpuFamilyPkg SmmCpuFeaturesLib.
// The Ia32CpuFamilyPkg CPU_HOT_PLUG_DATA differs by UefiCpuPkg of IedBase.
// Thus, this globalmHotPlugDataIedBasetakes care of the differences between structure defintions in this file.
//
UINT32  mHotPlugDataIedBase;
  ...
  )
{
  ...
  mSmmPpamEnable = FALSE;
  if ((IsNiftyRockSupported ()) &&  PcdGetBool (PcdPpamEnable)) {
    DEBUG ((DEBUG_INFO, "Nifty Rock feature supported\n"));
    mSmmPpamEnable = TRUE;
  }
  ...
  //
  DEBUG ((DEBUG_INFO, "PcdCpuSmmMsrSaveStateEnable     = %d\n", mSmmMsrSaveStateEnable));
  DEBUG ((DEBUG_INFO, "PcdCpuSmmProtectedModeEnable    = %d\n", mSmmProtectedModeEnable));
  DEBUG ((DEBUG_INFO, "PcdCpuSmmUseSmmEnableIndication = %d\n", mSmmEnableIndication));
  DEBUG ((DEBUG_INFO, "PcdCpuSmmUseDelayIndication     = %d\n", mSmmUseDelayIndication));
  DEBUG ((DEBUG_INFO, "PcdCpuSmmUseBlockIndication     = %d\n", mSmmUseBlockIndication));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "CpuSmmSaveStateInMsr         = %d\n", mSaveStateInMsr));
  ...
  )
{
  ...
  if (mIEDEnabled) {
    if (!mIEDBaseInitialized) {
      if (mIEDRamBase != 0) {
        ...
        //
        mHotPlugDataIedBase = mIEDRamBase;
      } else {
        //
        // If IED base address is not set, we assume it is located at end of SMM range
        //
        mHotPlugDataIedBase = CpuHotPlugData->SmrrBase + CpuHotPlugData->SmrrSize;
        //
  ...
  //
  if (mHotPlugDataIedBase != (UINT32) -1) {
    HaswellRelocateIedBase (CpuIndex, mHotPlugDataIedBase, IsMonarch);
  }
  if (IsSmrrSupported ()) {
  ...
  //
  AsmCpuid (CPUID_STRUCTURED_EXTENDED_FEATURE_FLAGS, NULL, NULL, NULL, &RegEdx);
  ...
  )
{
  ...
  if (mSmmSupovrStateLock) {
    ...
    for (Index = 0; Index < gSmst->NumberOfCpus; Index++) {
      if (mSmmPpamEnable) {
        gSmmSupovrStateLockData = EFI_MSR_SMM_SUPOVR_STATE_LOCK_PAGING_STATE_ENABLE_BIT + EFI_MSR_SMM_SUPOVR_STATE_LOCK_SMBASE_ENABLE_BIT;
        ...
        *(UINT64 *)(UINTN)(mSmBase[Index] + (UINTN)&gSmmSupovrStateLockData - (UINTN)gcSmmFeatureSmiHandlerTemplate + SMM_HANDLER_OFFSET) = EFI_MSR_SMM_SUPOVR_STATE_LOCK_PAGING_STATE_ENABLE_BIT + EFI_MSR_SMM_SUPOVR_STATE_LOCK_SMBASE_ENABLE_BIT;
      }
    }
