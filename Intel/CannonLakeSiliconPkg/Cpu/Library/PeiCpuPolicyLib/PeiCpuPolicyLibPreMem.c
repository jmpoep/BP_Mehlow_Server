  ...
  )
{
  ...
  ********************************/
  CpuSecurityPreMemConfig->BiosSize               = (7 * SIZE_1KB);      /// BIOS Size in KB
  CpuSecurityPreMemConfig->BiosGuard              = CPU_FEATURE_ENABLE;
  CpuSecurityPreMemConfig->EnableC6Dram           = CPU_FEATURE_ENABLE;
  if (CpuSecurityPreMemConfig->EnableC6Dram && IsC6dramSupported (CpuSecurityPreMemConfig->EnableC6Dram)) {
  ...
  )
{
  ...
  ********************************/
  CpuOverclockingPreMemConfig->OcLock = CPU_FEATURE_ENABLE; // Default enable OC Lock
}
