...
@par Specification Reference:
  System Management BIOS (SMBIOS) Reference Specification v3.1.1
  dated 2017-Jan-12 (DSP0134)
...
GLOBAL_REMOVE_IF_UNREFERENCED PROCESSOR_FAMILY_FIELD ProcessorFamilyField[] = {
  { "Intel(R) Core(TM) i9",  0xCF },                       ///< This will be part of the future SMBIOS specification. @todo Use enum from MdePkg when "ProcessorFamilyIntelCoreI9" is added.
  ...
  )
{
  ...
  //
  Status = MailboxRead (MAILBOX_TYPE_OC, MAILBOX_BCLK_FREQUENCY_CMD, &BclkFrequency, &MailboxStatus);

  if (EFI_ERROR (Status) || (MailboxStatus != EFI_SUCCESS)) {
    DEBUG ((DEBUG_INFO, "(SMBIOS) BCLK frequency message failed, Status = %X\n", Status));
    DEBUG ((DEBUG_INFO, "(SMBIOS) Setting to default BCLK frequency.\n"));
    BclkFrequency = 100;
  } else {
    //
    // Mailbox command returns BCLK in KHz, divide by 100 for MHz
    //
  ...
  switch (CpuSku) {
    case EnumCpuTrad:
      SmbiosProcessorInfoData.ProcessorUpgrade = ProcessorUpgradeSocketLGA1151;
      break;

    case EnumCpuUlt:
#ifdef CPU_CFL
      SmbiosProcessorInfoData.ProcessorUpgrade = SMBIOS_TYPE4_SOCKET_BGA1528; ///< @todo: set to value of ProcessorUpgradeSocketBGA1528, once the enum is available in MdePkg
#else
      SmbiosProcessorInfoData.ProcessorUpgrade = SMBIOS_TYPE4_SOCKET_BGA1510; ///< @todo: set to value of ProcessorUpgradeSocketBGA1510, once the enum is available in MdePkg
#endif
      break;

    case EnumCpuHalo:
#ifdef CPU_CFL
      SmbiosProcessorInfoData.ProcessorUpgrade = ProcessorUpgradeSocketBGA1440;
#else
      SmbiosProcessorInfoData.ProcessorUpgrade = ProcessorUpgradeOther; ///< @todo: set to value of ProcessorUpgradeSocketBGA1287, once it is assigned by DMTF
#endif
      break;

    case EnumCpuUlx:
      SmbiosProcessorInfoData.ProcessorUpgrade = SMBIOS_TYPE4_SOCKET_BGA1392; ///< @todo: set to value of ProcessorUpgradeSocketBGA1392, once the enum is available in MdePkg
      break;
