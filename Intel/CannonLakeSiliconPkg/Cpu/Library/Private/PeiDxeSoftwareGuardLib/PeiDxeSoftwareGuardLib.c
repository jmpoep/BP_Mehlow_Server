  ...
  )
{
  ...
  if (Cpuid.RegEcx & BIT30) {
    if (mCpuSgxConfig->SgxLCP == SGX_LCP_UNLOCKED) {
      Msr.Uint64 = AsmReadMsr64 (MSR_IA32_FEATURE_CONTROL);
      ...
      if (IsBspInt) {
        DEBUG ((DEBUG_INFO, "SGX Launch Control Configuration is unlocked.\n"));
      }
    } else {
      if (IsBspInt) {
        DEBUG ((DEBUG_INFO, "SGX Launch Control Configuration is locked.\n"));
      }
    }
  } else {
    if (IsBspInt) {
      DEBUG ((DEBUG_INFO, "SGX Launch Control Configuration is not supported.\n"));
    }
  }
}
  ...
  )
{
  ...
  if (mCpuSgxConfig->SgxLCP == SGX_LCP_LOCKED) {
    AsmWriteMsr64(MSR_IA32_SGXLEPUBKEYHASH0, mCpuSgxConfig->SgxLEPubKeyHash0);
    AsmWriteMsr64(MSR_IA32_SGXLEPUBKEYHASH1, mCpuSgxConfig->SgxLEPubKeyHash1);
    AsmWriteMsr64(MSR_IA32_SGXLEPUBKEYHASH2, mCpuSgxConfig->SgxLEPubKeyHash2);
    AsmWriteMsr64(MSR_IA32_SGXLEPUBKEYHASH3, mCpuSgxConfig->SgxLEPubKeyHash3);
  }
}
  ...
  )
{
  ...
  if (IsSgxSupported ()) {
    ...
    if (TxtInfoHob->Data.BiosAcmBase != 0 && TxtInfoHob->Data.ChipsetIsTxtCapable != 0 && TxtInfoHob->Data.ProcessorIsTxtCapable != 0) {
      if (CpuTxtPreMemConfig->SinitSvn != 0) {
        DEBUG((DEBUG_INFO, "SINIT SE SVN from SINIT ACM header: %4x\n", CpuTxtPreMemConfig->SinitSvn));
        SInitSvn = (UINT8 )CpuTxtPreMemConfig->SinitSvn;
      } else if (mCpuSgxConfig->SgxSinitDataFromTpm != 0) {
