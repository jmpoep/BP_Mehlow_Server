...
/**
  Detect if C6DRAM enabled or not by writing to the PCODE mailbox.
  If C6DRAM is supported and the desired state is enabled, then it
  will be enabled.
  ...
  )
{
  ...
  if (PrmrrSize == 0) {
    if (!(AsmReadMsr64 (MSR_PRMRR_PHYS_MASK) & B_MSR_PRMRR_PHYS_MASK_LOCK)) {
      ...
      AsmWriteMsr64 (MSR_PRMRR_PHYS_MASK, B_MSR_PRMRR_PHYS_MASK_LOCK);
    }
  } else {
    ...
    ///
    if (!(AsmReadMsr64 (MSR_PRMRR_PHYS_MASK) & B_MSR_PRMRR_PHYS_MASK_LOCK)) {
      AsmWriteMsr64 (MSR_PRMRR_PHYS_BASE, PrmrrBase | CACHE_WRITEBACK | B_MSR_PRMRR_CONFIG_BIT);
  ...
  )
{
  ...
  if (!IsSecondaryThread ()) {
    ...
    /// The valid bit on MSR 0x1F5 will be set later by the microcode
    ///
    if (PrmrrBase == 0) {
