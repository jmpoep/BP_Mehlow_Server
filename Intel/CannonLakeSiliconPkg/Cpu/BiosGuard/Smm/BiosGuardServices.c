...
GLOBAL_REMOVE_IF_UNREFERENCED CPU_NVS_AREA_PROTOCOL             *mCpuNvsAreaProtocol = NULL;
  ...
  )
{
  ...
  if (mBiosGuardProtocol != NULL) {
    ...
    mCpuNvsAreaProtocol->Area->BiosGuardMemAddress     = mBiosGuardMemAddress;
    mCpuNvsAreaProtocol->Area->BiosGuardMemSize        = (UINT8) RShiftU64 (mBiosGuardMemSize, 20);
    mCpuNvsAreaProtocol->Area->BiosGuardIoTrapAddress  = PchIoTrapContext.Address;
    mCpuNvsAreaProtocol->Area->BiosGuardIoTrapLength   = (UINT8) PchIoTrapContext.Length;
  }
}
  ...
  )
{
  ...
  /// Locate Cpu Nvs
  ///
  Status = gBS->LocateProtocol (&gCpuNvsAreaProtocolGuid, NULL, (VOID **) &mCpuNvsAreaProtocol);
  ASSERT_EFI_ERROR (Status);
