  ...
  )
{
  ...
  if (Status == EFI_ALREADY_STARTED) {
    ...
    Status = PeiServicesReInstallPpi (OldStatusCodeDescriptor, mStatusCodePpiList);
    ASSERT_EFI_ERROR (Status);
    ...
    Status = PeiServicesReInstallPpi (OldRscHandlerDescriptor, mRscHandlerPpiList);
    ASSERT_EFI_ERROR (Status);
  ...
  //
  // This is first time starting from flash.
  //
  // SIPO-CNL-0002: Override End.
