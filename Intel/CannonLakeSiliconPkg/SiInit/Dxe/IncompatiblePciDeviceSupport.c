  ...
  * (VOID **) Configuration = NULL;
  ...
  if ((UINT16) DeviceId == PciSegmentRead16 (PCI_SEGMENT_LIB_ADDRESS(SA_SEG_NUM, SA_IGD_BUS, SA_IGD_DEV, SA_IGD_FUN_0, PCI_DEVICE_ID_OFFSET))) {
    ...
    if (SiConfigHobData->CsmFlag == 1) {
      AcpiPtr = AllocateZeroPool (2 * sizeof (EFI_ACPI_ADDRESS_SPACE_DESCRIPTOR) + sizeof (EFI_ACPI_END_TAG_DESCRIPTOR));
      if (AcpiPtr == NULL) {
      ...
      AcpiPtr->GenFlag = 0;
      AcpiPtr->SpecificFlag = EFI_ACPI_MEMORY_RESOURCE_SPECIFIC_FLAG_CACHEABLE;
      AcpiPtr->AddrSpaceGranularity = 32;
      AcpiPtr->AddrRangeMin = 0;
      AcpiPtr->AddrRangeMax = OLD_ALIGN;
      AcpiPtr->AddrTranslationOffset = PCI_BAR_IDX0;
      AcpiPtr->AddrLen = 0;
      AcpiPtr++;
      ...
      if (EFI_ERROR (Status) || (MiscDxeConfig == NULL)) {
        return EFI_OUT_OF_RESOURCES;
      }
      if (MiscDxeConfig->EnableAbove4GBMmio == 1) {
